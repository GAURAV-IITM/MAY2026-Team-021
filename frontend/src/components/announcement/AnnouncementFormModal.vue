  ...
  <Modal :is-open="isOpen" :title="mode === 'edit' ? 'Edit Announcement' : 'Create Announcement'" title-id="announcement-form-title" @close="close">
    <form class="announcement-form" novalidate @submit.prevent="submit">
      ...
      <fieldset class="announcement-form__publication">
        ...
        <label v-if="mode === 'create'"><input v-model="form.publication" type="radio" value="published" /> Publish now</label>
        <label><input v-model="form.publication" type="radio" value="scheduled" /> Schedule</label>
      </fieldset>

      <div v-if="form.publication === 'scheduled'" class="form-field" :class="{ 'form-field--error': errors.scheduledAt }">
        <label class="form-label" for="announcement-publish-at">Publish date and time</label>
        <input id="announcement-publish-at" v-model="form.scheduledAt" class="form-control" type="datetime-local" :min="minimumDateTime" />
        <p v-if="errors.scheduledAt" class="form-help">{{ errors.scheduledAt }}</p>
      </div>
...
const form = reactive({ title: '', body: '', category: 'general', audience: 'all_students', publication: 'published', scheduledAt: '', expiresAt: '', isImportant: false })
...
function reset() {
  ...
  form.audience = item?.audience || props.audiences[0]?.value || 'all_students'
  form.publication = item?.status === 'scheduled' ? 'scheduled' : item?.status === 'draft' ? 'draft' : 'published'
  form.scheduledAt = item?.status === 'scheduled' ? toLocalInput(item.scheduledAt) : ''
...
function validate() {
  ...
  if (form.publication === 'scheduled' && !form.scheduledAt) errors.scheduledAt = 'Select when this announcement should be published.'
  const publishTime = form.publication === 'scheduled' ? new Date(form.scheduledAt) : new Date()
  if (form.publication === 'scheduled' && publishTime <= new Date()) errors.scheduledAt = 'Scheduled publish time must be in the future.'
...
function submit() {
  ...
  emit('save', {
    ...
    expiresAt: form.expiresAt
      ...
      : null,
    expectedUpdatedAt: props.announcement?.updatedAt || undefined,
  })
}
